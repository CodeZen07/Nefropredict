...
def crear_pdf(datos, recoms, alertas, medico):
    ...
    pdf.cell(0, 12, "REPORTE MÉDICO CARDIORRENAL", ln=True, align='C')
    ...
    pdf.cell(95, 8, f"Médico: Dr. {medico}", border=1, ln=True)
    pdf.ln(8)
    
    # Resultados clínicos
    pdf.set_font("Arial", 'B', 13)
    pdf.set_fill_color(230, 240, 250)
    pdf.cell(0, 10, "RESULTADOS CLÍNICOS", ln=True, fill=True)
    pdf.set_font("Arial", '', 11)
    
    resultados = [
        ("Presión Sistólica", f"{datos.get('sys', 'N/A')} mmHg"),
        ("TFG (Función Renal)", f"{datos.get('tfg', 'N/A')} ml/min/1.73m²"),
        ("Potasio (K+)", f"{datos.get('potasio', 'N/A')} mEq/L"),
        ("FEVI (Fracción Eyección)", f"{datos.get('fevi', 'N/A')}%"),
        ("Horas de Sueño", f"{datos.get('sleep', 'N/A')} horas/día"),
        ("Nivel de Estrés", datos.get('stress', 'N/A'))
    ]
    ...
    if alertas:
        ...
        pdf.cell(0, 8, "⚠ ALERTAS CLÍNICAS", ln=True)
        pdf.set_text_color(0, 0, 0)
    ...
    categorias = {
        'clinico': ('MANEJO CLÍNICO', (0, 100, 0)),
        'dieta': ('INTERVENCIÓN NUTRICIONAL', (139, 69, 19)),
        'estilo': ('MODIFICACIÓN DE ESTILO DE VIDA', (0, 51, 102)),
    ...
    pdf.multi_cell(0, 5, "AVISO LEGAL: Este reporte es una herramienta de apoyo clínico basada en guías KDIGO 2024 y AHA/ACC 2023. No sustituye el juicio clínico profesional ni la evaluación individualizada del paciente. Todas las decisiones terapéuticas deben ser validadas por el médico tratante considerando el contexto clínico completo del paciente.")
...
if menu == "🔬 Nueva Consulta":
    ...
    if st.session_state.analisis_listo:
        ...
        with col_p1:
            ...
            parametros = ['Presión\nSistólica', 'Potasio\n(K+)', 'Horas\nSueño']
            valores = [d['sys'], d['potasio']*25, d['sleep']*15]  # Normalizado para visualización
